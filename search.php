...
function createArrayForJson($points, $hard_search, $conn){
	...
	foreach ($points as $key => $value){
		...
		$sql2 = "SELECT link FROM urls WHERE id = $value[url_id] ORDER BY link asc";
	...
	foreach ($hard_search as $value){
		...
		$sql2 = "SELECT link FROM urls WHERE id = $value ORDER BY link asc";
      ...
      ?>
        <section class="searchHeader">
          <div class="container">
            <form class="searchForm" method="get" action="">
              <input class="searchInput" autocomplete="off" name="searchInput" id="search" type="text" value=<?php echo "'".$placeholder."'"; ?>>
              <button type="submit" class="searchButton--icon"><i class="fa fa-search" aria-hidden="true"></i></button>
            </form>
          </div>
        </section>
        ...
        <section class="searchMain">
          <div class="container">
			...
			?>
              <p><?php echo $no; ?> results (<?php if(isset($total_time)){ echo $total_time; }?> seconds)</p>
            </div>
            ...
            <div class="searchItems">
              <?php
               ...
               foreach ($result as $key => $value) {
             	...
             	echo '<h2><a href="'.$value['url'].'">'.$title.'</a></h2>';
             	echo '<p class="searchItem-link">'.$value['url'].'</p>';
             	echo '<p class="searchItem-desc">'.substr($value['text'], 0, 300).'...</p>';
             	echo '</div>';
